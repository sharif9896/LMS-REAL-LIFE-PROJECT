    ...
    <div className="bg-gray-100 min-h-screen font-sans text-gray-800 flex items-center justify-center p-4">
      <div className="w-full max-w-4xl mx-auto">
        <header className="text-center mb-10">
          <h1 className="text-4xl font-bold text-gray-700">Ṣending to Students</h1>
          <p className="text-lg text-gray-500 mt-2">Upload and manage your class Assignments.</p>
        </header>

        {/* Form Section */}
        <div className="bg-white p-8 rounded-2xl shadow-lg mb-10">
          <h2 className="text-2xl font-semibold mb-6 text-gray-700">Add Downloaded Document</h2>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* File Upload Input */}
            <div>
              <label htmlFor="pdfFile" className="block text-sm font-medium text-gray-600 mb-2">
                PDF File
              </label>
              <input
                type="file"
        ...
        <div className="bg-white p-8 rounded-2xl shadow-lg">
          <h2 className="text-2xl font-semibold mb-6 text-gray-700">Uploaded Documents</h2>
          <div className="space-y-4">
            {entries.length > 0 ? (
              entries.map((entry) => (
                <div key={entry.id} className="bg-gray-50 p-4 rounded-lg flex flex-wrap items-center justify-between gap-4 border border-gray-200">
                  <div className="flex-1 min-w-0">
                    <p className="text-lg font-semibold text-indigo-700 truncate">{entry.className}</p>
                    <p className="text-sm text-gray-500">{entry.department}</p>
                    <p className="text-sm text-gray-600 mt-1 truncate">
                      <span className="font-medium">File:</span> {entry.pdfFile.name}
                    </p>
                  </div>
                  <a
                    href={entry.fileUrl}
              ...
              <p className="text-center text-gray-500 py-4">No documents have been uploaded yet.</p>
            )}
          </div>
        </div>
      </div>
