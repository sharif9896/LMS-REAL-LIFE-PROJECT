...
const CourseImportExport = () => {
  ...
  const handleExport = async () => {
    setLoading(true);
    const res = await axios.get(`${BACKEND_URL}api/course/export`, {
      responseType: "blob",
    });
    setTimeout(() => {
      setLoading(false);
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BACKEND_URL}api/course/import`, {
        method: "POST",
        headers: {
          "Content-Type":
  ...
  return (
    <>
      <div style={{ padding: 24, fontFamily: "sans-serif" }}>
        <h2>Courses & Exams Excel Import / Export</h2>
        <div className="flex justify-between items-center mb-1">
          <p className="text-gray-400">
            <b>Expected Excel headers:</b>  Coursename, Course_program,
            Exam_Title, Exam_Duration, <br /> Exam_Qs_Limit Course_exam
          </p>
        </div>
      </div>
