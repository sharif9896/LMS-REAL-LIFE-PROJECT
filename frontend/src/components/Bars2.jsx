...
const StudentFileCard = () => {
  ...
  const handleDownload = (id) => {
    ...
    link.href = `${BACKEND_URL}downloads/${id}`; // Adjust based on your backend/static file setup
      ...
      {file ? (
        ...
        <div>
          <div className="w-auto h-55 flex justify-center items-center">
            <img className="w-60" src="gdf.png" alt="" />
          </div>
          <p className="w-auto flex text-center justify-center items-center">
            - Oop's, Does'nt have an any Announcement yet! -
          </p>
        </div>
      )}
    </>
